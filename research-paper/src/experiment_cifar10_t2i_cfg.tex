...
\begin{table}[H]
    ...
    \begin{tabular}{|c|c|c|}
        \hline
        \textbf{Guidance Scale ($w$)} & \textbf{FID Score} $\downarrow$ & \textbf{Accuracy (\%)}$\uparrow$ \footnotemark \\
        \hline
    ...
    \end{tabular}
\end{table}
\footnotetext{Accuracy is measured using a ResNet-18 classifier adapted from ImageNet but not fine-tuned on CIFAR-10. The classifier's first convolution and final layer were modified for 32$\times$32 images and 10 classes, but no additional training was performed. Thus, the absolute accuracy values are underestimated, but the relative trend shows that higher guidance scales yield better prompt adherence, as expected.}
