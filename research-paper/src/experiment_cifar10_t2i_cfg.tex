...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.7\textwidth,height=0.85\textheight,keepaspectratio]{../figures/experiments/samples_w10.png}
    \caption[Generated CIFAR-10 images for all 10 classes at guidance scale $w=10$]{Generated CIFAR-10 images for all 10 classes using guidance scale $w=10$. Each row shows 4 samples for a class: airplane, automobile, bird, cat, deer, dog, frog, horse, ship, and truck. The model demonstrates the ability to generate diverse natural images conditioned on text prompts.}
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.95\textwidth,keepaspectratio]{../figures/experiments/confusion_matrices.png}
    \caption[Per-class generation accuracy shown via confusion matrices for all guidance scales]{Normalized confusion matrices for CIFAR-10 generation across all guidance scales ($w \in \{0, 2, 5, 10\}$). Rows represent the intended class from the text prompt, columns represent the classifier's prediction. Diagonal values indicate correct generation. The matrices reveal that some classes (e.g., truck, ship) are easier to generate correctly than others (e.g., cat, dog).}
